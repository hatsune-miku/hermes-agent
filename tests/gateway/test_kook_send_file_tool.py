    ...
    class Bot:
        def __init__(self, token=""):
            ...
            self.client = SimpleNamespace(
                gate=SimpleNamespace(request=AsyncMock()),
...
def test_send_file_behind_link_downloads_and_sends_to_channel(monkeypatch, tmp_path):
    ...
    result = json.loads(handle_send_file_behind_link({"file_path": "https://example.com/x.png", "file_name": "renamed.png"}))

    assert result == {"msg_id": "ch-msg"}
    assert downloads["url"] == "https://example.com/x.png"
    assert downloads["dest"].name == "renamed.png"
    channel.send.assert_awaited_once_with("https://cdn.kook.example/x.png", type=fake.MessageTypes.FILE)
...
def test_send_file_behind_link_uses_dm_when_dm_chat(monkeypatch, tmp_path):
    ...
    result = json.loads(handle_send_file_behind_link({"file_path": "https://example.com/x.png", "file_name": "name.png"}))

    assert result == {"msg_id": "dm-msg"}
    channel.send.assert_not_called()
    user.send.assert_awaited_once_with("https://cdn.kook.example/x.png", type=fake.MessageTypes.FILE)
...
def test_send_file_behind_link_requires_chat_id(monkeypatch, tmp_path):
    ...
    result = json.loads(handle_send_file_behind_link({"file_path": "https://example.com/x.png", "file_name": "x.png"}))
...
def test_send_file_behind_link_rejects_non_http(monkeypatch, tmp_path):
    ...
    result = json.loads(handle_send_file_behind_link({"file_path": "ftp://nope/x", "file_name": "x.png"}))
